...
try {
    const dataRPGDBPath = path.resolve('./system/store/dataRPG.json')
    if (!fs.existsSync(dataRPGDBPath)) {
    fs.writeFileSync(dataRPGDBPath, JSON.stringify({}));
    console.log('Archivo dataRPG.json creado.')
    }
    ...
    global.dataRPG = (await import(path.resolve('./system/rpg/dataRPG.js'))).default;
    global.dataRPGDB = JSON.parse(fs.readFileSync(dataRPGDBPath))
    ...
    global.BotName = '@KAZE AI'
    global.ptn = '●'
    global.author = '○ KAZE-AI - 𝟸𝟺/𝟽'
            ...
            await m.messages.forEach(async m => {
                try {
                    ...
                    if (m.message && m.key) {
                        ...
                        m.IS = (roles) => {
                        const trustedUsers = config.botConfig.trustedUsers || {};
                        const userRoles = trustedUsers[m.sender.number] || {};
                        if (typeof roles === 'string') {
                        return userRoles[roles] === true
                            }
                        if (Array.isArray(roles)) {
                        return roles.some(role => userRoles[role] === true)
                            }
                        return false
                        }
